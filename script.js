document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch('http://localhost:5001/api/ping')
      .then(res => res.json())
      .then(data => {
        console.log('✅ Frontend connected to backend:', data.message);
      })
      .catch(err => {
...
function saveJournal() {
    ...
    if (entry.trim() !== "") {
        ...
        .then(res => res.json())
        .then(data => {
            document.getElementById("savedMessage").textContent = "✅ Journal entry saved to server!";
        })
        .catch(err => {
            document.getElementById("savedMessage").textContent = "❌ Error saving entry.";
